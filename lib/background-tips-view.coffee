...
class BackgroundTipsElement
  ...
  isUserOnCodeScreen: ->
    return atom.project.remoteftp? && atom.project.remoteftp.isConnected()

  shouldBeAttached: ->
    @workspaceCenter.getPanes().length is 1 and not @workspaceCenter.getActivePaneItem()? and @isUserOnCodeScreen()
